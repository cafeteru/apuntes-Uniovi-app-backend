package es.uniovi.apuntesuniovi.infrastructure.messages

/**
 * Defines the messages that the subjects exceptions return
 */
object SubjectMessages {
  const val INVALID_ID = "error.subject.invalid.id"
  const val INVALID_JSON = "error.subject.invalid.json"
  const val INVALID_SUBJECT_TYPE = "error.subject.invalid.subject.type"
  const val LIMIT_NAME = "error.subject.limit.name"
  const val NOT_FOUND = "error.subject.not.found"
  const val NULL = "error.subject.null"
}